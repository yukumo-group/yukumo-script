...
_clib-win goos goarch cc out ldflags cflags cgo_ldflags:
    ...
    $ld = '{{cgo_ldflags}}'; if ($ld -like '*-L{{macos_stub_dir}}*') { $ld = $ld.Replace('-L{{macos_stub_dir}}', '-L'+((Resolve-Path '{{macos_stub_dir}}').Path)) }; $env:CGO_ENABLED='1'; $env:GOOS='{{goos}}'; $env:GOARCH='{{goarch}}'; $env:CC='{{cc}}'; if ('{{cflags}}') { $env:CGO_CFLAGS='{{cflags}}' } else { $env:CGO_CFLAGS=$null }; if ($ld) { $env:CGO_LDFLAGS=$ld } else { $env:CGO_LDFLAGS=$null }; if ('{{cflags}}') { go build -tags {{clib_tags}} -buildmode=c-shared -trimpath -ldflags="{{ldflags}}" -gcflags="{{gc_debug}}" -o "{{out}}" ./cmd/clib } else { go build -tags {{clib_tags}} -buildmode=c-shared -trimpath -ldflags="{{ldflags}}" -o "{{out}}" ./cmd/clib }; $dir = Split-Path -Parent "{{out}}"; $base = [IO.Path]::GetFileNameWithoutExtension("{{out}}"); if (($base -ne 'yukumo') -and (Test-Path "$dir/$base.h")) { Move-Item -Force "$dir/$base.h" "$dir/yukumo.h" }

[unix]
_clib-unix goos goarch cc out ldflags cflags cgo_ldflags:
    mkdir -p "$(dirname "{{out}}")"
    ld="{{cgo_ldflags}}"; case "$ld" in *-L{{macos_stub_dir}}*) ld="$(echo "$ld" | sed "s|-L{{macos_stub_dir}}|-L$(cd {{macos_stub_dir}} && pwd)|")" ;; esac; if [ -n "{{cflags}}" ]; then CGO_ENABLED=1 GOOS={{goos}} GOARCH={{goarch}} CC="{{cc}}" CGO_CFLAGS="{{cflags}}" CGO_LDFLAGS="$ld" go build -tags {{clib_tags}} -buildmode=c-shared -trimpath -ldflags="{{ldflags}}" -gcflags="{{gc_debug}}" -o "{{out}}" ./cmd/clib; else CGO_ENABLED=1 GOOS={{goos}} GOARCH={{goarch}} CC="{{cc}}" CGO_LDFLAGS="$ld" go build -tags {{clib_tags}} -buildmode=c-shared -trimpath -ldflags="{{ldflags}}" -o "{{out}}" ./cmd/clib; fi; dir="$(dirname "{{out}}")"; base="$(basename "{{out}}" | sed 's/\.[^.]*$//')"; if [ "$base" != "yukumo" ] && [ -f "$dir/$base.h" ]; then mv -f "$dir/$base.h" "$dir/yukumo.h"; fi
